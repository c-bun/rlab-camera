"""Display bring-up test harness for the HUB75 illumination panel (run on demand).

The panel refreshes continuously in the background (hub75 core1 thread); this loop just
draws patterns and calls show(). Cycles the 8 hues at several densities plus
geometry/orientation checks, printing each pattern to the REPL so it can be verified by
eye. Does not touch BLE -- run it to check the display alone when the BLE app (main.py)
is in the way.

Push with mpremote and run it explicitly (main.py, the BLE app, is what runs on boot):
    mpremote connect /dev/ttyACM0 fs cp hub75.py :hub75.py
    mpremote connect /dev/ttyACM0 fs cp display_test.py :display_test.py
    mpremote connect /dev/ttyACM0 run display_test.py
"""
