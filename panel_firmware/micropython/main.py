...
After rendering each command the panel notifies a one-byte "I'm on" ack (1 = lit,
0 = cleared) on a second, notify-only status characteristic, so the Pi's capture path can
wait for the panel to actually be on before it integrates a frame.

This mirrors ``app/illumination/protocol.py`` on the Pi (``SERVICE_UUID``,
``COMMAND_CHAR_UUID``, ``STATUS_CHAR_UUID``, ``to_payload``) and replaces the old
CircuitPython ``panel_firmware/code.py`` -- keep the UUIDs and payload layout in sync with
protocol.py.
...
# Must match SERVICE_UUID / COMMAND_CHAR_UUID / STATUS_CHAR_UUID in
# app/illumination/protocol.py.
...
_STATUS_UUID = bluetooth.UUID("5f1d0003-9d6f-4c1e-8b2a-2a5f3c9e7a10")
...
# write_gatt_char(..., response=False); write=True also accepts a confirmed write (the
# capture path uses response=True). capture=True makes ``written()`` return (connection,
# data) so we read the payload directly.
...
# Notify-only status characteristic: after the panel renders a command we notify the
# central with a one-byte "I'm on" ack (1 = lit, 0 = cleared), so the Pi's capture path can
# wait for the panel to actually be on before it integrates a frame.
_status = aioble.Characteristic(_service, _STATUS_UUID, notify=True)
...
def apply_command(data):
    ...
    Returns the rendered state byte for the ack notification: 1 = lit, 0 = cleared;
    None if the command was malformed (nothing rendered, no ack).
    """
        ...
        try:
            while connection.is_connected():
                ...
                try:
                    ...
                    continue
                state = apply_command(data)
                if state is not None:
                    # Ack that the panel has actually rendered this command ("I'm on"),
                    # so the Pi's capture path can proceed. A disconnect mid-notify must
                    # not crash the loop -- it just re-advertises.
                    try:
                        _status.notify(connection, bytes([state]))
