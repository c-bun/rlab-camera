...
- **LED panels (separate microcontrollers):** each is a **Raspberry Pi Pico W running CircuitPython** (`panel_firmware/code.py`) wired to a Waveshare RGB-matrix panel; it acts as a BLE peripheral the Pi connects to. This firmware is flashed to the board, not run from this repo. `bleak` (unlike picamera2) *does* import on macOS, so the illumination code needs no import guard — but there is no panel hardware off-Pi, so illumination still sits behind the same mock-backed abstraction for dev and tests.
...
- The panel wire protocol and GATT UUIDs are shared between `app/illumination/protocol.py` and `panel_firmware/code.py` — change both together.
